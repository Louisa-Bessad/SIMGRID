...
http://www.loria.fr/~quinson/Research/SMP
...
- Work on the segfault of communication.c could come from:
  ...
  - Looking for this with Valgrind:
    - process_fcntl_call (sys_memory.c:398): we try to reach the file descriptor indexed by arg
      but in the case of proc this file descriptor can be allocated at annother index
      than "arg->fd" in its list of file descriptor
...
*** Blocking points
- see l.282 and 284 of this file
*** Planned work
- Maybe add a variable in process_descriptor to keep the size of the list of the file descriptor
  owned by a process. Because all the for loops are going from 0 to MAX_FD but fd_list is dynamic 
  so it is possible that we continue on the loop till 1024 except there is few files
...
- Implemented options of fcntl:
  ...
  - F_GETFL & F_SETFL
  - F_SETLK & F_SETLKW
- Using the right registers in ptrace_get_registers function according to the architecture
- Index all the flags for fcntl syscall in sys_fcntl.c and args_trace.c
- Find the root of the segfault: the main process of Simterpose sends datas to Simterpose
  that return a NILL pointer. And then we try to access this pointer -> segfault
*** Blocking poins
- I don't really understand what is done in sys_memory.:269 -> 275
- Valgrind try to read or write in files that was never opened (proc->fd_list[arg->fd] = nil):
...
- Is it not dangerous to assign unsigned long of reg_s struct to a variable
  of type int (file_desc->fd)?
- What is the register orig_rax?
- How handle the difference between access mode and status flags in our struct of file
*** Planned work
- modify the struct for file to make the difference between access mode and status flags maybe
**** DONE complete all allocation of fd_descriptor with the new field
