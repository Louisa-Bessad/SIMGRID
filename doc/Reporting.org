...
- recvmsg and recvfrom create an infinite waiting when we del the sysarg structures
*** Planned work
...
- Possible bench (overhead made by simterpose to catch syscalls, on big and little files too)
- Try to code time functions or one other mediation

** 03.08.2015 -> 07.08.2015
*** Things done
- Mediation par translation
  - Fcntl syscall test file (the two last tests are weird)
  - Open, close syscalls ok
*** Blocking points
*** Planned work
- Use the "time" command to get execution time
