...
\centering\begin{tikzpicture}[mindmap,
  ...
  % Applied area: Simterpose

  \begin{scope}[mindmap, concept color=orange, text=white]
    \node [concept] {Simterpose}[clockwise from=-5] 
      child {node [concept] (port) {\textbf{port\_des} \\ \*bind\_socket \\ struct infos\_socket}}
      child {node [concept] (host) {\textbf{simterpose\_host}\\ port\\xbt\_dict\_t}};
  \end{scope}

  % Applied area: Processus

  \begin{scope}[mindmap, concept color=red,text=white]
    \node [concept] at (-5,-15) {Processus}
    child [grow=-10, level distance=160]
    {node [concept] (str) {\textbf{stream\_t} \\ msg\_sem\_t \& msg\_process\_t}}
    child [grow=20] 
    {node [concept] (procdesc) {\textbf{pipe\_end\_\*} \\ process\_descriptor\_t}}
    child [grow=110] 
    {node [concept] (pipe) {\textbf{pipe\_t}, xbt\_dynar\_t}}
    child [grow = 30]
    {node [concept] (fddesc) {\textbf{fd\_descriptor\_t} \\ process\_descriptor\_t \& stream\_t \& pipe\_t}};          
  \end{scope}
