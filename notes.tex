...
\subsection{Les AS de syscall\_process}
  ...
  \item syscall\_read done
  \item syscall\_write done
  \item syscall\_open done devrait être en premier, il gère comme il faut les descripteurs (l'émulation / simulation) avec exit et read write
  \item syscall\_close done
  \item syscall\_poll\_pre revoir
  \item syscall\_poll\_post done
  \item syscall\_pipe\_post done
  \item syscall\_select\_pre revoir devrait être avec les ``polls''
  \item syscall\_dup2\_post done
  \item syscall\_socket done
  \item syscall\_connect\_pre revoir
  \item syscall\_connect\_post revoir
  \item syscall\_accept
  \item syscall\_sendto\_pre
  \item syscall\_sendto\_post
  \item syscall\_recvfrom\_pre
  \item syscall\_recvfrom\_post
  \item syscall\_sendmsg
  \item syscall\_reccvmsg\_pre
  \item syscall\_recvmsg\_post 
  \item syscall\_shutdown\_pre
  \item syscall\_shutdown\_post
  \item syscall\_bind\_pre
  \item syscall\_bind\_post
  \item syscall\_listen
  \item syscall\_getpeername\_pre
  \item syscall\_setsockopt\_pre
  \item syscall\_setsockopt\_post
  \item syscall\_getsockopt\_pre
  \item syscall\_getsockopt\_post
  \item syscall\_clone
  \item syscall\_execve
  \item syscall\_exit
  \item syscall\_fcntl
  \item syscall\_creat\_post
  \item syscall\_brk
  \item {\color{white} baba}
  \item \textit{process\_read\_out\_call} done
  \item \textit{process\_send\_call}
  \item \textit{process\_recvfrom\_out\_call}
  \item \textit{process\_read\_out\_call}
  \item \textit{process\_close\_call} done
  \item \textit{process\_fcntl\_call}
  \item \textit{process\_listen\_call}
  \item \textit{process\_accept\_out\_call}
  \item \textit{process\_connect\_in\_call} revoir
  \item \textit{process\_handle}
  \end{itemize}
